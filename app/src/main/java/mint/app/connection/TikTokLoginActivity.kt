...
class TikTokLoginActivity : ComponentActivity() {
    ...
    private fun checkCookies() {
        if (finished) return
        val cookies = CookieManager.getInstance().getCookie("https://www.tiktok.com") ?: return
        val parsed = cookies.split(";").associate {
            val parts = it.trim().split("=", limit = 2)
            parts.first() to parts.getOrElse(1) { "" }
        }
        if (parsed.containsKey("sid_tt")) {
            finished = true
            pollJob?.cancel()
            ConnectionPreferences.saveTikTokSession(this, parsed)
            val username = parsed["unique_id"]?.takeIf { it.isNotBlank() }
            ConnectionPreferences.setTikTokUsername(this, username)
            Toast.makeText(this, "TikTok linked successfully", Toast.LENGTH_SHORT).show()
            setResult(RESULT_OK)
            finish()
        }
    }
...
private fun LoginScreen(webView: WebView, loading: Boolean, onClose: () -> Unit) {
    Column(
        ...
        Row(
            ...
            Text(
                text = stringResource(R.string.login_tiktok_title),
